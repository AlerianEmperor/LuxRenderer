...
const float sigma_a_ = 0.005;//0.006;//0.006;//0.09
const float sigma_s_ = 0.09;//0.036;//0.009;//0.05
...
struct Scene
{
	...
	vec3 inifinite_light_direction = (-1, -0.1, 0.1);
		...
		for (int i = 0; i < 60; ++i)
		{
			...
			if (hit_color_alpha(n, new_ray, rec, &color, alpha))
			{
				...
				if (mats[mtl].type == SubSurface_Scattering_type)
				{
					...
					if (cos_incident < 0.0f)
					{
						vec3 new_hit_point = new_ray.o + new_ray.d * rec.t * 1.002f;
						...
						if (hit_color_alpha(n, in_volume_ray, new_rec, &new_color, new_alpha))
						{
							...
							float sample_distance = -log(randf()) / lambda;
							...
							{
								//T *= expf(-new_rec.t);
								vec3 new_point_out_side = in_volume_ray.o + in_volume_ray.d * new_rec.t * 1.002f;
								new_ray = Ray(new_point_out_side, new_ray.d);
								continue;
							}
						}
